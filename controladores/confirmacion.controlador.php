 ...
 <?php
    if ($peticionAjax) {
        require_once "../modelos/confirmacion.modelo.php";
     ...
     *
     */
    class confirmacionControlador extends confirmacionModelo
    {

        // insertar
        public function CtrInsertarConfirmacion()
        {

            $hoy              = date('Y-m-d');
            $fechanacimiento = $_POST['confechanacimiento'];
            $nombres           = $_POST['connombres'];
            $apellidos             = $_POST['conapellidos'];
            $nombrepadre           = $_POST['connombrespadre'];
            $apellidopadre    = $_POST['conapellidospadre'];
            $telefonopadre  = $_POST['contelefonopadre'];
            $nombremadre  = $_POST['connombresmadre'];
            $apellidomadre  = $_POST['conapellidosmadre'];
            $telefonomadre      = $_POST['contelefonomadre'];
            $nombrepadrino      = $_POST['connombrespadrino'];
            $apellidopadrino         = $_POST['conapellidopadrino'];
            $nombremadrina     = $_POST['connombresmadrina'];
            $apellidomadrina        = $_POST['conapellidosmadrina'];
            $fechacelebracion        = $_POST['confechacelebracion'];
            $nombreministro      = $_POST['connombresministro'];
            $apellidoministro        = $_POST['conapellidosministro'];
            $name       = $_FILES['conpdfbautizo']['name'];
            $guardado = $_FILES['conpdfbautizo']['tmp_name'];
            $pdf       = 'confirmaciones/' . $name;
            $usuario          = 1;
            ...
            $datos = [
                ...
                "apellidopadrino"        => $apellidopadrino,
                "nombremadrina"        => $nombremadrina,
                "apellidomadrina"      => $apellidomadrina,
                "nombreministro"        => $nombreministro,
                "apellidoministro"       => $apellidoministro,
                "pdf"       => $pdf,
                "fechanacimiento" => $fechanacimiento,
                "usuario"          => $usuario,
            ];
            $saveimg = mainModel::guardarArchivo('./../archivos/confirmaciones', $guardado, $name);
            if ($saveimg == true) {
                $insertar = confirmacionModelo::MdlInsertarConfirmacion($datos);
                if ($insertar->rowCount() >= 1) {

                    $alerta = [
                        "Alerta" => "limpiar",
                        "Titulo" => "Insertar confirmación",
                    ...
                    $alerta = [
                        ...
                        "Texto"  => "No se registro la confirmación",
                        "Tipo"   => "error",
                    ];
                }
            } else {
        ...
        /* paginador */
        public function CtrlPaginadorConfirmaciones($pagina, $registros, $consulta)
        {
            ...
            if ($consulta != "") {
                ...
                $query = "SELECT * FROM datosactividad where id_tipoActividad=4 ORDER by id_datosActividad ASC LIMIT $inicio, $registros";
            }

            // echo ($query);
            $datos    = $conexion->query($query);
            $datos    = $datos->fetchAll();
            $total    = $conexion->query("SELECT COUNT(*) FROM datosactividad where id_tipoActividad=4");
            ...
            $tabla .= '
                <table class="table table-hover text-center">
                   <thead>
                        ...
                        <th class="text-center">CERTIFICADO BAUTIZO</th>
            ...
            if ($total >= 1 && $pagina <= $Npaginas) {
                foreach ($datos as $key => $value) {
                    $tabla .= '
                        <tr>
                            ...
                            <td>
                                <a target="_blank" href="' . SERVERURL . 'archivos/' . $value['act_pdfceertificadobautizo'] . '" class="btn btn-info btn-raised btn-xs">
                                    <i class="zmdi zmdi-print"></i>
                                </a>
                            </td>
                            <td>
                            <a href="' . SERVERURL . 'confirmacionesedit/' . mainModel::encryption($value['id_datosActividad']) . '" class="btn btn-success btn-raised btn-xs">
                                <i class="zmdi zmdi-refresh"></i>
                            </a>
                        </td>
                            <td>
                                <form class="FormularioAjax" method="POST" data-form="delete" action="' . SERVERURL . 'ajax/confirmacionAjax.php">
                                <input type="hidden" name="confirmacionDelete" value="' . mainModel::encryption($value['id_datosActividad']) . '"> </input>
                ...
                if ($total >= 1) {
                    $tabla .= '
                    <tr>
                        <td><a href="' . SERVERURL . 'confirmacioneslista/" class="btn btn-sm btn-info btn-raised"> Haga clic para regargar listado</a></td>
                    </tr>';
                } else {
                    $tabla .= '
                    <tr>
            ...
            if ($total >= 1 && $pagina <= $Npaginas) {
                ...
                if ($pagina == 1) {
                    $tabla .= '<li class="disabled page-item"><a href="' . SERVERURL . 'confirmacioneslista/' . ($pagina - 1) . '"><i class="zmdi zmdi-arrow-left "></i></a></li>';
                } else {
                    $tabla .= '<li class="page-item"><a href="' . SERVERURL . 'confirmacioneslista/' . ($pagina - 1) . '"><i class="zmdi zmdi-arrow-left "></i></a></li>';
                }

                for ($i = 1; $i <= $Npaginas; $i++) {
                    if ($pagina == $i) {
                        $tabla .= '<li class="active page-item"><a href="' . SERVERURL . 'confirmacioneslista/' . $i . '">' . $i . '</a></li>';
                    } else {
                        $tabla .= '<li class="page-item"><a href="' . SERVERURL . 'confirmacioneslista/' . $i . '">' . $i . '</a></li>';
                    }
                }
                //paginacion fecha derecha
                if ($pagina == $Npaginas) {
                    $tabla .= '<li class="disabled page-item"><a href="' . SERVERURL . 'confirmacioneslista/' . ($pagina - 1) . '"><i class="zmdi zmdi-arrow-right "></i></a></li>';
                } else {
                    $tabla .= '<li class="page-item"><a href="' . SERVERURL . 'confirmacioneslista/' . ($pagina + 1) . '"><i class="zmdi zmdi-arrow-right "></i></a></li>';
                }
                $tabla .= '</ul>
                </nav>';
            }
